...
public class CheckWithdrawCoinServiceImpl implements CheckWithdrawCoinService {
	...
	public void checkWithdrawCoinOrder(Integer uid, AuthLevel userAuthLevel, String assetCode, BigDecimal amount) {
		...
		BigDecimal withdrawDailyArreadyUsed = withdrawCoinService.getUserDailyWithdrawedCoinValue(uid, assetCode,
				currentDayBegin.getTime(), new Date());
		// 1.5.1版本
		...
		if (null == withdrawLevelDailyLimit || WITHDRAW_LEVEL_STATE_VALUE.equals(withdrawLevelDailyLimit)) {
			throw new AppException(WithdrawalsCodeConst.LESS_WITHDRAWAL_LEVEL_ERROR, "转出币种数量超过用户对应实名认证等级限制");
		}
		...
		if (null != withdrawLevelDailyLimit && 0 < withdrawLevelDailyLimit.compareTo(BigDecimal.ZERO)) {
			...
			if (BigDecimalUtils.isBigger(amount, withdrawLevelDailyLimit.subtract(withdrawDailyArreadyUsed))) {

				throw new AppException(WithdrawalsCodeConst.WITHDRAWAL_COIN_ADILY_MAX_OVER_ERROR, "转出值超过对应等级的每日限额",
