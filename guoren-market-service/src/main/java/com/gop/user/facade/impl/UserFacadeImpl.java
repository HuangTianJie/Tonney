...
import com.gop.domain.enums.AccountKind;
...
public class UserFacadeImpl implements UserFacade {
	...
	public void checkLoginPassword(Integer uid, String loginPasswod) {
		...
		if (!CryptoUtils.verify(user.getLoginPassword(), MD5Util.genMD5Code(loginPasswod), user.getLoginSalt())) {
	...
	public void checkPayPassword(Integer uid, String payPasswod) {
		...
		CheckPayPasswordLockedDto checkPayPasswordLockedDto = userPayPasswordService.CheckPayPasswordLockedTimes(uid);

		// 验证支付密码
		if (!CryptoUtils.verify(user.getPayPassword(), MD5Util.genMD5Code(payPasswod), user.getPaySalt())) {
	...
	@Override
	@Transactional
	public User createUser(String email, String phone, String password, String payPwd, Integer invitId, String nickname,
			Integer brokerId) {
		User user = userService.registerUser(email, phone, password, payPwd, invitId, nickname, brokerId);
		try {
			List<ConfigAsset> avaliables = userAccountFacade.getAvailableAssetCode();
			for (ConfigAsset configAsset : avaliables) {

				userAccountFacade.createAccount(user.getUid(), brokerId, configAsset.getAssetCode().toString(),
						AccountKind.MASTER.toString());
				if (configAsset.getCurrencyType().equals(CurrencyType.COIN)) {
					coinTransferFacade.createCoinAddress(configAsset.getAssetCode().toString(), user.getUid(),
							configAsset.getAssetCode().toString());
				}

			}

		} catch (Exception e) {
			log.error("调用资产Facade创建初始账户异常userId={},eMessage=" + e.getMessage(), user.getUid(), e);
	...
	public void checkUserGoogleCode(Integer uid, String email, String code) {
		...
		if (null != userGoogleCodeConfig && UserGoogleCodeFlagType.ON.equals(userGoogleCodeConfig.getFlag())) {
	...
	@Override
	public User reRegistrationUserCreate(String email, String phone, String password,String loginsalt, Integer invitId, String nickname,
			Integer brokerId) {
		
		User user = userService.preRegisterUser(email, phone, password, loginsalt, invitId, nickname, brokerId);
		try {
			List<ConfigAsset> avaliables = userAccountFacade.getAvailableAssetCode();
			for (ConfigAsset configAsset : avaliables) {
