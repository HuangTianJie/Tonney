...
public class ConfigSymbolServiceImpl implements ConfigSymbolService {
	...
	@Override
	public List<ConfigSymbol> getAllAssetCode1() {
		try {
			return configSymbolMapper.getAllAssetCode1();
		} catch (Exception e) {
			log.error("查询交易对列表失败", e);
			throw new AppException(CommonCodeConst.SERVICE_ERROR,"查询交易对列表失败");
		}
	...
	@Override
	public List<ConfigSymbol> getAllAssetCode2(String assetCode1) {
		try {
			return configSymbolMapper.getAllAssetCode2(assetCode1);
		} catch (Exception e) {
			log.error("查询交易对列表失败", e);
			throw new AppException(CommonCodeConst.SERVICE_ERROR,"查询交易对列表失败");
		}
