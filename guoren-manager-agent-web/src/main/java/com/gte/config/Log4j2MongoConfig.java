...
import com.gop.config.LogContext;
...
public class Log4j2MongoConfig {
	...
	public static MongoClient mongoClient() {
		...
		try {
			String startActive = LogContext.init();
			String active = StringUtils.isEmpty(startActive) ?  "" : "-" + startActive;
			hostports = ResourceUtils.get("mongodbForLog" + active, "mongodb.hostports");
			username = ResourceUtils.get("mongodbForLog" + active, "mongodb.username");
			password = ResourceUtils.get("mongodbForLog" + active, "mongodb.password");
			database = ResourceUtils.get("mongodbForLog" + active, "mongodb.database");
			maxConnect = ResourceUtils.get("mongodbForLog" + active, "mongodb.maxConnect");
			maxWaitThread = ResourceUtils.get("mongodbForLog" + active, "mongodb.maxWaitThread");
			maxTimeOut = ResourceUtils.get("mongodbForLog" + active, "mongodb.maxTimeOut");
			maxWaitTime = ResourceUtils.get("mongodbForLog" + active, "mongodb.maxWaitTime");
