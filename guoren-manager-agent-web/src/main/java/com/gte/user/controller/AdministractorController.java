package com.gte.user.controller;
...
public class AdministractorController {
	...
	public JSONObject login(@AuthForHeader AuthContext context) {
		...
		Agent agent = agentService.getAgentByAccount(account);
		...
		//校验账户谷歌验证码
		administratorsFacade.checkManagerGoogleCode(agent.getAdminId(), agent.getMobile(), googleCode);
		LoginSession session = new LoginSession();
		session.setUserId(agent.getAdminId());
		context.setLoginSession(session);
		String token = tokenHelper.generateToken(agent.getAdminId());
		JSONObject json = new JSONObject();
		json.put("token", token);
		json.put("role", agent.getRole().toString());
		
		boolean boundGoogle = managerGoogleCodeConfigService.countManagerGoogleCodeConfigByAdminId(agent.getAdminId());
		json.put("boundGoogle", boundGoogle);
		boolean setPwdFlag = managerPasswordOperRecordService.countSetManagerPassword(agent.getAdminId());
		...
		managerOperLogService.recordManagerOperLog(agent.getAdminId(), ip, ManagerOperType.LOGIN, "");
		return json;
	}
