...
public class FaultBarrier {
	...
	public Object exp(HttpServletRequest request, AppException ex) {
		...
		if(locale!= null && locale.toString().equals("zh_CN")
				&& ex.getErrCode().equals(CommonCodeConst.FIELD_ERROR)
				&& StringUtils.isNotEmpty(ex.getCodeMessage())){
			WebApiResponse webApiResponse = new WebApiResponse();
			webApiResponse.setCode(ex.getErrCode());
			webApiResponse.setMsg(ex.getCodeMessage());
			webApiResponse.setData(ex.getAttach());
